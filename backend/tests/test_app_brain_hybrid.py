...
async def test_retrieve_app_sections_from_tree():
    ...
    async with AsyncSessionLocal() as db:
        ...
        db.add(app)
        await db.flush()
        cdoc = AppCorpusDoc(app_id=app.id, file_id=uuid.uuid4(), name="pricing.pdf",
                            index_status="done")
        # file_id FK → use a real file row to satisfy FK
    ...
    async with AsyncSessionLocal() as db:
        sections = await retrieve_app_sections(app_id, "what is the price cap?", top_k=5, db=db)
...
async def test_ingest_corpus_doc_builds_tree_and_chunks():
    ...
    async with AsyncSessionLocal() as db:
        ...
        tree = (await db.execute(select(AppDocTree).where(AppDocTree.app_id == app_id))).scalar_one_or_none()
